...
The lifecycle value gate did not justify a new metadata ontology. Across 12
frozen questions in three vault shapes, existing conventions answered all 12,
and a proposed lifecycle layer made zero additional answers deterministic.
KB therefore ships no `kb_role` field, lifecycle resolver or API, lifecycle
CLI, compatibility diagnostic, or metadata migration. Current and historical
plan routing remains derived from existing note type, path, and status.
