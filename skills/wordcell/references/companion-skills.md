...
A companion skill handles one recurring Wordcell ritual that benefits from a
discriminating trigger and an explicit operating contract. It composes with
the public `wordcell` skill. It does not register code at runtime, execute vault
...
planning, percolation, refresh, and validation in the main `wordcell` skill. Propose
...
they retain. Markdown and Git remain authoritative Wordcell state. Generated
...
Define the narrow checks that establish the intended result. After material Wordcell
edits, review percolation candidates and run the appropriate catalog-aware
check. Parallel lanes use `wordcell check --no-catalog`; one integrating agent owns a
managed catalog refresh.

## Composition boundary

Call the installed `wordcell` command or its public package interfaces only when the
