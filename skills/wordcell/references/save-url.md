...
```sh
wordcell capture show "$KB_ROOT/articles/<slug>"
wordcell capture verify "$KB_ROOT/articles/<slug>" --verify-assets --json
wordcell capture diff "$KB_ROOT/articles/<slug>" --repo . --ref main --json
```
...
With Wordcell installed, build the pinned Rust metadata-search helper and backfill every saved external URL into a separate tool-owned sidecar:
...
Read [the platform routing guide](url-platforms.md) when selecting or explaining a route. Use `wordcell adapters --json` when software needs the installed capability matrix.
