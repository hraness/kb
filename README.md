...
```sh
bun add --global @hraness/kb@0.19.3
kb --help
```
...
`@hraness/kb@0.19.3` registry release only when the command is missing. Verify it
...
CLI from the immutable `@hraness/kb@0.19.3` npm version.

The public skills CLI reads `skills/kb/` from the repository. The immutable
`0.19.3` npm package includes the same tree under
...
```sh
npm install --global --ignore-scripts @hraness/kb@0.19.3
kb --help
```
...
```sh
bun add --exact @hraness/kb@0.19.3
```
